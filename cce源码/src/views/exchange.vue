  ...
  <div class="exchange allFlex column">
    ...
    <div class="pool">
      ...
      <div class="bottom allFlex boxBg">
        ...
        <p>{{ type ? 'CCE' : 'USDT' }}</p>
        <img src="../assets/img/exchange.png" class="jh" @click="type=!type;buyNumber=0;">
        <img src="../assets/img/icon_cce.png" v-if="!type">
        <img src="../assets/img/icon_usdt.png" v-if="type">
        <p>{{ type ? 'USDT' : 'CCE' }}</p>
      </div>
    ...
    <div class="join allFlex column boxBg">
      <div class="allFlex ts"><span class="Wcolor">兑换的数量</span>
        <span class="Wcolor" v-if="!type">当前汇率：1 USDT={{ ccetousdt }} CCE</span>
        <span class="Wcolor" v-if="type">当前汇率：1 CCE={{ usdttocce }} USDT</span>
      </div>
      <div class="input">
      ...
      <p class="balance">可用余额：<span class="Wcolor">{{type ? balanceObj.cce : balanceObj.usdt}}</span> {{type ? 'CCE' : 'USDT'}}</p>
      <div class="btn PFcolor font-Five center" @click="flashcash">立即兑换</div>
    </div>
    <!-- 兑换记录 -->
    <div class="padding titleNav record-title blueBg allFlex">
      <span class="font-Eight titleColor">兑换记录</span>
    </div>
    ...
    <ul class="record-ul">
      ...
      >
        <div class="allFlex column">
          <span class="Wcolor font-Six">兑换{{item.type===1 ? 'CCE' : 'USDT'}}</span>
          <span class="textColor font-Four">{{ item.time |formatDates }}</span>
        </div>

        <div class="allFlex column">
          <span class="font-Eight PFcolor right">+{{ item.type===1 ? item.ccenumber : item.usdtnumber }} {{item.type===1 ? 'CCE' : 'USDT'}}</span>
          <span class="textColor font-Four right">消耗{{ item.type===1 ? item.usdtnumber : item.ccenumber }}{{item.type===0 ? 'CCE' : 'USDT'}}</span>
        </div>
...
import {flashcashFn, getflashcashrecFn, getPriceFn, getrankingFn, userbalanceFn} from "@/api/axios";
  ...
  methods: {
    ...
    async getBalance() {
      ...
      if (res) {
        ...
        this.balanceObj.usdt = (Math.floor(Number(Global.numberUtil(res.enusdtbal, 6))*10000)/10000).toFixed(4)
      }
    },
    async flashcash(){
      if (this.buyNumber<=0){
      ...
      let res = await flashcashFn(0,paramType,Number(this.buyNumber)*1000000);
      if (res){
        this.$toast('兑换成功');
      }
    },
    async getflashcashrec(){
      let res = await getflashcashrecFn(1);
      if (res){
        let obj=[]
        res._type.forEach((itme,key)=>{
          obj.push({type:Number(Global.toBigNumber(res._type[key])),cceprice:Number(Global.numberUtil(res.cceprice[key],6)).toFixed(2),ccenumber:Number(Global.numberUtil(res.ccenumber[key],6)).toFixed(4),usdtprice:Number(Global.numberUtil(res.usdtprice[key],6)).toFixed(2),usdtnumber:Number(Global.numberUtil(res.usdtnumber[key],6)).toFixed(4),time:Number(Global.toBigNumber(res.time[key]))})
        })
        this.recordArr = obj
