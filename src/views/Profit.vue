<template>
  <div class="profit padding">
    ...
    <!-- 累计收益 -->
    <ul class="border ztBj profit-balance allFlex column radius">
      <li class="f1 allFlex column center">
        <span class="textColor font-Four">累计收益</span>
    ...
    <!-- 领取 -->
    <ul class="border ztBj profit-balance2 allFlex column radius">
      <li class="f1 allFlex column center">
        <span class="textColor font-Four">待领取收益</span>
      ...
      <li>
        <div class=" Wcolor font-Five get-btn center" @click="receive">
          一键领取
        </div>
      </li>
    </ul>
    ...
    <ul class="record-ul">
      ...
      >
        <div class="allFlex column">
          <span class="Wcolor font-Six">{{ navArr[index] }}</span>
          <span class="textColor font-Four">{{ item.time | formatDates }}</span>
        </div>

        <span class="font-Eight blueColor"
          >+{{ item.number }} {{ typeArr[item.type] }}</span
        >
      </li>
    </ul>
  </div>
...
export default {
  ...
  methods: {
    async init() {
      let res = await profitPage();
      this.getBalance(res);
      this.getprofitres(res);
      this.getprofit(res);
      console.log(res);
    },
    async changeRecordArr(key) {
      this.index = key;
      if (key == 0) {
        this.recordArr = this.recordArr1.sort((a, b) => b.time - a.time);
      } else if (key == 1) {
        this.recordArr = this.recordArr2.sort((a, b) => b.time - a.time);
      } else {
        this.recordArr = this.recordArr3.sort((a, b) => b.time - a.time);
      }
      // console.log(this.recordArr)
    },
    getBalance(res) {
      if (res) {
        this.balanceObj.cce = Number(
          Global.numberUtil(res.walletcce, 6)
        ).toFixed(2);
        this.balanceObj.usdt = Number(
          Global.numberUtil(res.walletusdt, 6)
        ).toFixed(2);
      }
    },
    getprofitres(res) {
      if (res) {
        ...
        this.changeRecordArr(this.index);
        // console.log(obj)
        let sum1 = 0;
        let sum2 = 0;
        let sum3 = 0;
