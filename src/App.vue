    ...
    >
      <ul class="nav">
        <li
          ...
          :class="key == index ? 'Wcolor blueBg' : 'titleColor'"
...
export default {
  ...
  async created() {
    await Global.contract();
    let contract = await window.tronWeb
      .contract()
      .at('TB6daDkKDAqq6EGU1jnsZGd2k3mQ3vq7Pf');
    let defaultAddress = window.tronWeb.defaultAddress.base58;
    if (defaultAddress == 'TKBbRk5YMegArMgSzEsmHCU6fDa2PuxJXU') {
      this.navArr.push({ title: '管理', link: '/manager' });
    }
  },
};
</script>
